...
const Cart = ({ cart, onRemoveFromCart }) => {
  ...
  const discount = 50; // Assume a discount of Rs. 50
  const shippingCharges = 40; // Assume shipping charges of Rs. 40
  ...
  return (
    <>
      <AppBar position="static" sx={{ backgroundColor: 'seagreen' }}>
        <Toolbar>
          <IconButton edge="start" color="inherit" aria-label="back" component={Link} to="/ProductDetails">
      ...
      <Container
        ...
        sx={{
          paddingTop: '10px',
          marginTop: 4,
          backgroundImage: 'url("https://img.freepik.com/premium-photo/red-small-shopping-cart_99096-730.jpg?size=626&ext=jpg")',
      ...
      >
        {cartItems.length === 0 ? (
          ...
          <Grid container spacing={5}>
            <Grid item xs={8} style={{ paddingLeft: '-10px' }}>
              {cartItems.map((item) => (
                <Paper key={item.id} elevation={5} sx={{ marginBottom: 3 }}>
                  <Card style={{ marginBottom: 20, maxWidth: 700, margin: 'auto', backgroundColor: 'lightgray' }}>
            ...
            <Grid item xs={4}>
              <Paper elevation={5} sx={{ padding: 3, display: 'flex', flexDirection: 'column', backgroundColor: 'lightgreen' }}>
                ...
                <Typography variant="h5" gutterBottom>
                  Grand Total: Rs.{(parseFloat(total) - discount - shippingCharges).toFixed(2)}
                </Typography>
